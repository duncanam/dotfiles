...
export default function worker(pi: ExtensionAPI) {
  ...
  const architect = `\nYou are the technical lead. Investigate the user's goal and existing changes, choose an approach, delegate jobs, resolve blockers and review results. Use normal read/write/edit/Bash, gh and configured extension tools as needed. Delegate substantive implementation to the implementor; coordinate any supporting edits to avoid conflicts. Use ai_directive assign with a task, constraints and acceptance criteria, leaving local coding decisions to the implementor. Yield after dispatch; use timer reminders for check-ins rather than polling. Use guide for feedback or corrections in the same cycle, including after done; ping only requests status. After done, independently review the work with appropriate files, tests or CI rather than relying on transcripts. Accept, guide corrections or assign a new job using your judgment; explain the outcome and verification limits, then wait.`;
  ...
  if (role === 'architect') {
    ...
    pi.registerTool({
      ...
      async execute(_id, params) {
        ...
        return { ...output(params.kind === 'status' ? 'Status sent. Continue implementation.' : 'Paused; wait for architect guidance.', params), ...(params.kind !== 'status' ? { terminate: true } : {}) };
      },
    });
  }
}
