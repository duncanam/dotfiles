...
export default function mockProvider(pi: ExtensionAPI) {
  ...
  pi.registerProvider('pair-fixture', {
    ...
    models: ['architect', 'implementor'].map((id) => ({ id, name: id, reasoning: true, input: ['text'],
    ...
    streamSimple(model, context, options) {
      ...
      queueMicrotask(async () => {
        try {
          ...
          if (model.id === 'architect') {
            assert.match(context.systemPrompt ?? '', /You are the ARCHITECT/);
            ...
            assert.match(context.systemPrompt ?? '', /You are the IMPLEMENTOR/); assert.doesNotMatch(history, /MAIN_ONLY_SENTINEL/);
            assert.ok(step < 6);
          ...
          output.stopReason = 'error'; output.errorMessage = String(error);
